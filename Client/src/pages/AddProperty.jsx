...
//         console.log(optionsListResponse)
//       } catch (error) {
...
const AddProperty = () => {
  ...
  const navigate = useNavigate();
  const [seller_id, setSeller] = useState(null); // Seller ID state
  const [propertyDetails, setPropertyDetails] = useState({
    property_type: "land",
    address: "",
    city: "",
    ...
    listing_date: "",
    description: "",
    seller: null, // Start with seller as null
  ...
  // Session checking to ensure seller ID is present
  useEffect(() => {
    const checkSession = async () => {
      try {
        const sellerResponse = await sendRequest({
  ...
  // Synchronize seller ID with property details
  useEffect(() => {
    if (seller_id) {
      setPropertyDetails((prevDetails) => ({
        ...prevDetails,
  ...
  const validateForm = () => {
    ...
    if (propertyDetails.property_type === "house") {
    ...
    try {
      ...
      if (response.status === 401) {
        ...
        setPropertyDetails({
          ...
          listing_date: "",
          description: "",
          seller: seller_id, // Reset seller to the current seller ID
          bedrooms: "", // Reset house-specific fields
          bathrooms: "",
        });
  ...
  return (
    <div>
      ...
      <div className="max-w-lg space-y-4">
        ...
        {propertyDetails.property_type === "house" && (
          <>
            <Input
              label="Bedrooms"
              type="number"
        ...
        {showModal && (
          <Modal message="Property Added Successfully" onClose={closeModal} />
        )}
      </div>
    </div>
  );
